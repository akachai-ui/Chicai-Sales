...
export default function PlannerPage() {
  ...
  // Update Stop Objective
  const handleObjectiveChange = (stopId: string, objective: string) => {
    const updatedStops = plan.stops.map((s) => (s.id === stopId ? { ...s, objective } : s));
    const updated = { ...plan, stops: updatedStops };
    updateAndSavePlan(updated);
  };

  // Update Stop Planned Time
  const handlePlannedTimeChange = (stopId: string, plannedTime: string) => {
    const updatedStops = plan.stops.map((s) => (s.id === stopId ? { ...s, plannedTime } : s));
    const updated = { ...plan, stops: updatedStops };
    updateAndSavePlan(updated);
  };
                ...
                return (
                  <React.Fragment key={stop.id}>
                    ...
                    <div className="p-3.5 sm:p-4 space-y-3 text-xs">
                      {/* Meta Info & Planned Time */}
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-slate-600">
                        {/* Time Picker */}
                        <div className="flex items-center space-x-1.5 bg-slate-100/90 px-2.5 py-1 rounded-xl border border-slate-200">
                          <Clock className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                          <span className="text-[11px] font-bold text-slate-700">เวลานัด:</span>
                          <input
                            type="time"
                            value={stop.plannedTime || ''}
                            onChange={(e) => handlePlannedTimeChange(stop.id, e.target.value)}
                            className="bg-transparent text-xs font-black text-slate-900 focus:outline-none w-16"
                          />
                          <div className="hidden sm:flex space-x-1 pl-1 border-l border-slate-300">
                            {['09:30', '13:30', '15:00'].map((t) => (
                              <button
                                key={t}
                                type="button"
                                onClick={() => handlePlannedTimeChange(stop.id, t)}
                      ...
                      {/* Interactive Objective Section */}
                      <div className="p-3 bg-gradient-to-br from-blue-50/70 via-indigo-50/40 to-slate-50 rounded-2xl border border-blue-200/80 space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-1.5 text-blue-900 font-extrabold text-[11px] uppercase tracking-wider">
                            <Target className="w-4 h-4 text-blue-600 shrink-0" />
                            <span>วัตถุประสงค์การเข้าพบ (Visit Objective) *</span>
                          </div>
                          <span className="text-[10px] text-blue-600/80 font-medium hidden sm:inline">
                            พิมพ์เองหรือกดเลือกปุ่มด้านล่างได้ทันที
                          </span>
                        </div>
                        ...
                        <div className="relative">
                          <input
                            type="text"
                            placeholder="พิมพ์ระบุวัตถุประสงค์การเข้าพบ เช่น สาธิตเครื่องกรองน้ำมัน, ติดตามใบเสนอราคา..."
                            value={stop.objective || ''}
                            onChange={(e) => handleObjectiveChange(stop.id, e.target.value)}
                            className="w-full px-3 py-2 rounded-xl border border-blue-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-xs font-bold text-slate-800 shadow-2xs"
                          />
                        </div>

                        {/* Quick Preset Buttons */}
                        <div className="flex flex-wrap gap-1 pt-0.5">
                          {COMMON_OBJECTIVES.map((preset) => {
                            const isSelected = stop.objective === preset;
                            return (
                              <button
                                key={preset}
                                type="button"
                                onClick={() => handleObjectiveChange(stop.id, preset)}
                                className={`px-2.5 py-1 rounded-lg text-[10.5px] font-bold transition-all flex items-center space-x-1 ${
                                  isSelected
                                    ? 'bg-blue-600 text-white shadow-xs'
                                    : 'bg-white/90 hover:bg-blue-50 text-slate-700 border border-blue-200/60 hover:border-blue-400'
                                }`}
                              >
